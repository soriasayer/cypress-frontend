...
export class SmartTablePage{
  ...
  addNewRecordWithFirstAndLastName(firstName, LastName, userName, email, age){
    ...
    cy.get('thead').find('tr').eq(2).then(tableRow => {
     ...
     cy.get('tbody tr').first().find('td').then(tableColunms => {
      cy.wrap(tableColunms).eq(2).should('contain', firstName)
      cy.wrap(tableColunms).eq(3).should('contain', LastName)
      cy.wrap(tableColunms).eq(4).should('contain', userName)
      cy.wrap(tableColunms).eq(5).should('contain', email)
      cy.wrap(tableColunms).eq(6).should('contain', age)
    })
  }
